...
export default function PortalLayout({ nav, label, footerLink }) {
      ...
      {nav.map((n, i) => {
        ...
        return (
          <React.Fragment key={n.to}>
            {showGroup && <div className="mt-4 mb-1 px-3 text-[10px] font-semibold uppercase tracking-wider text-slate-500">{n.group}</div>}
            <NavLink to={n.to} end={n.end} onClick={() => setOpen(false)}
              className={({ isActive }) => cn('flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition-colors', isActive ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-white')}>
  ...
  const Side = () => (
    <div className="flex h-full flex-col bg-[#0B1020] py-6 text-white">
      ...
      <div className="mt-6 flex-1 overflow-y-auto portal-sidebar-scroll"><Nav /></div>
